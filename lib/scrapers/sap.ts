...
// SAP SuccessFactors jobs scraped via Serper Google Jobs API.
// Filter by "via" field containing "SAP" or "SuccessFactors".

const SERPER_URL = "https://google.serper.dev/jobs";

const QUERIES = [
  "software engineer jobs successfactors",
  "data engineer jobs successfactors",
  "machine learning engineer site:jobs.sap.com",
];
...
async function querySerper(q: string): Promise<SerperJob[]> {
  ...
  if (!res.ok) throw new Error(`Serper SAP: HTTP ${res.status}`);

  const data = (await res.json()) as SerperResponse;
  return data.jobs ?? [];
}

function isSAPJob(job: SerperJob): boolean {
  const via = (job.via ?? "").toLowerCase();
  return via.includes("sap") || via.includes("successfactors");
}

export async function scrapeSAP(): Promise<ScrapedJob[]> {
  ...
  for (const q of QUERIES) {
    try {
      ...
      for (const job of rawJobs) {
        ...
        const applyUrl =
          job.apply_options?.find((o) =>
            o.title.toLowerCase().includes("sap") ||
            o.title.toLowerCase().includes("successfactors")
          )?.link ??
  ...
  if (errors.length > 0) {
    console.warn("[sap] partial errors:", errors.join(", "));
  }

  console.log(`[sap] fetched ${results.length} recent US tech jobs`);
  return results;
}
